...
除了命令行，本项目还内置了一个 Web 界面，把上述所有功能搬到网页上，开箱即用。

### 启动

```bash
pip install -r requirements.txt   # 确保已含 flask
python web_app.py
```
...
- **💬 问答**：基于 `docs/` 中文档回答问题，并在「参考来源」中展示命中片段与相似度。
- **🔍 检索**：输入查询，返回按相似度（cosine，越大越相关）排序的文档片段。
- **📄 文档**：查看 `docs/` 下的 txt 列表，支持上传新 .txt（上传后需点「初始化 / 重建索引」生效）。
- **🕘 历史**：查看本会话的问答记录。
- **⚙️ 设置**：调整 `chunk_size` / `chunk_overlap` / `search_k`。
- **顶部配置栏**：选择模型（DeepSeek / MiMo / OpenAI）、填写 API Key、一键初始化 / 重建索引。
- 文档加载、智能分割、向量检索、相似度评分、模型切换、参数自定义——全部覆盖。

> 说明：API Key 既可从界面填写，也可沿用 `.env`（见上文）。索引每次「初始化 / 重建」都会基于当前 `docs/` 重新生成。
